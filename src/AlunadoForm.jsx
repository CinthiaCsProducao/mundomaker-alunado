...
// ─── Gerar PDF do formulário preenchido ──────────────────────
function gerarPDFFormulario(dadosGerais, series, totalAlunos, responsavel, calendarioNome) {
  ...
  const seriesHtml = series.map(s => {
    ...
    return `
      <tr style="background:#f5f5f5;">
        <td colspan="3" style="padding:8px 14px;font-weight:700;font-size:12px;">${s.serie} — ${totalSerie} alunos</td>
      </tr>
      ${s.turmas.map(t => `
        <tr>
  ...
  win.document.write(`
    ...
    <meta charset="UTF-8">
    <title>Formulário de Alunado — ${dadosGerais.nome}</title>
    <style>
    ...
    <div class="section">
      ...
      <div class="sec-body">
        ...
        <div class="total-box">
          <div class="total-num">${totalAlunos.toLocaleString("pt-BR")}</div>
    ...
    <div class="section">
      ...
      <div class="sec-body">
        ...
        <div class="f-val" style="margin-top:4px;">${responsavel || "—"}</div>
      </div>
    </div>
    ...
    <div class="section">
      ...
      <div class="sec-body">
        ...
        <div class="f-val" style="margin-top:4px;">${calendarioNome}</div>
      </div>
    </div>
    ` : ""}
    </body></html>
  ...
  if (sucesso) return (
    <div style={S.successPage}>
      <div style={S.successCard}>
        ...
        <div style={{ fontSize: 28, fontWeight: 800, color: "#111", marginBottom: 8 }}>
          dados enviados!
        </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          <button
            style={{ ...S.btn, background: "#39DF18", color: "#111", padding: "12px 24px", fontSize: 13, fontWeight: 700, border: "none", borderRadius: 6, cursor: "pointer" }}
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div style={S.sectionBlock}>
            <div style={S.sectionHead}>
              ...
              <div style={S.sectionTitle}>Programa e Idioma do Material</div>
            </div>
            <div style={S.sectionBody}>
                  ...
                  <div style={{ display: "flex", flexWrap: "wrap", gap: 10, marginTop: 4 }}>
                    {["MakerLab Class", "MakerLab Oficina", "Inventores do Futuro"].map(p => (
                      <button key={p} type="button"
                        style={{
                          ...S.btn,
                          ...(dadosGerais.programa === p ? S.btnGreen : S.btnOutline),
                  ...
                  <div style={{ display: "flex", gap: 10, marginTop: 4 }}>
                    {["Português", "Inglês"].map(idioma => (
                      <button key={idioma} type="button"
                        style={{
                          ...S.btn,
                          ...(dadosGerais.idioma_material === idioma ? S.btnGreen : S.btnOutline),
                          fontSize: 13, padding: "10px 20px",
                        }}
            ...
            <div style={S.sectionBody}>
              ...
              {series.map((s, si) => (
                <div key={s.serie} style={{ border: "1.5px solid #e5e5e5", borderRadius: 4, marginBottom: 16, overflow: "hidden" }}>
                  ...
                  <div style={{ padding: 16 }}>
                    ...
                    {s.turmas.map((t, ti) => (
                      <div key={ti} style={{ display: "grid", gridTemplateColumns: "64px 1fr 1fr auto", gap: 8, marginBottom: 8, alignItems: "center" }}>
                        ...
                        <input style={S.input} type="number" min={0} value={t.num_alunos}
                          onChange={e => atualizarTurma(si,ti,"num_alunos",e.target.value)}
                          placeholder="0" />
                        <input style={S.input} value={t.professor_maker}
                          onChange={e => atualizarTurma(si,ti,"professor_maker",e.target.value)}
                    ...
                    {s.turmas.length < 16 && (
                      <button type="button"
                        style={{ ...S.btn, ...S.btnOutline, fontSize: 12, marginTop: 4 }}
              ...
              {series.length > 0 && (
                <div style={{ background: "#111", borderRadius: 4, padding: "20px 24px", marginTop: 8 }}>
                  <div style={{ fontSize: 36, fontWeight: 800, color: "#39DF18", lineHeight: 1 }}>
                    {totalAlunos}
                  </div>
            ...
            <div style={S.sectionBody}>
              <div style={S.uploadArea} onClick={() => document.getElementById("cal-input").click()}>
                ...
                {calendario ? (
                  <>
                    {calendarioPreview && (
                      <img src={calendarioPreview} alt="preview"
                        style={{ maxHeight: 100, borderRadius: 4, marginBottom: 10 }} />
                    )}
                    <div style={{ fontWeight: 700, color: "#111", fontSize: 14 }}>✓ {calendario.name}</div>
                    <div style={{ fontSize: 12, color: "#888", marginTop: 4 }}>
                      {(calendario.size/1024).toFixed(1)} KB — clique para trocar
                    </div>
                  </>
                ) : (
                  <>
